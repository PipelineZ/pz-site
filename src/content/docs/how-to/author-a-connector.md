...
:::note
`pz`'s ten builtin connectors (`localfiles`, `postgres`, `s3`, `sqlserver`, `azureblob`, `gcs`,
`http`, `mysql`, `sqlite`, `sftp`) are compiled directly into the CLI from the `pz` repository:
