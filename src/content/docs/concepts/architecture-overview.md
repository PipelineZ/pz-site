...
the ADRs under [decisions/](/decisions/) carry the full analysis for the data plane and
concurrency choices.
...
| 2 | Canonical data format | Arrow RecordBatch | IDataReader/DataTable/rows | zero-copy DuckDB boundary, off-heap, industry interchange ([ADR 0001](/decisions/0001-arrow-ingest-path/), [ADR 0002](/decisions/0002-arrow-egress-path/), [ADR 0004](/decisions/0004-batch-memory-pooling/)) |
...
| 7 | DAG inference | from ref()/source() at render time | SQL parsing | simple, exact, dbt-proven; DuckDB still validates the SQL — amended by [ADR 0006](/decisions/0006-watermark-expressions-in-pipeline-sql/), a scoped exception for incremental *config* (not edges) inferred from DuckDB-parsed `watermark()` comparisons |
| 8 | Batch sizing | byte-targeted (~32 MB) adaptive | fixed row counts | wide/narrow tables both hit the amortization sweet spot |
| 9 | Concurrency | channels + async + DAG limit; DuckDB self-parallel | custom thread pools | backpressure and bottleneck metrics for free; no lock-based data path ([ADR 0005](/decisions/0005-duckdb-connection-strategy/)) |
