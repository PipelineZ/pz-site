...
|---|---|
| `replace` | The destination is unchanged. A connector that also declares `Transactional` (`postgres`, `sqlserver`, `duckdb`, `ducklake`, `iceberg`) commits the whole overwrite as one transaction, snapshot, or catalog commit — `iceberg`'s replace lands as two new snapshots (a delete then an append), but neither reaches the catalog until both do, in one commit. A file or object-store connector (`localfiles`, `s3`, `azureblob`, `gcs`, `sftp`) writes to a temporary location and promotes it into place only on success. Either way there is no half-written destination to clean up: a retry sees exactly the pre-run state. |
