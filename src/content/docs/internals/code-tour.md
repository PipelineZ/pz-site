---
title: "Code tour"
description: "This page follows one pz run from the keystroke to the output files, naming the actual classes and files at every stop, for a contributor who understands what pz does but gets lost in the code."
sidebar:
  order: 6
---
...
actual classes and files at every stop, using the sample project that `pz init --template sample`
creates as the running example. No prior knowledge of the internals is assumed.
...
repetition is the design: one set of rails, verbs differ only in how far they ride them. See
[Execution internals](/internals/execution-internals/) for the full phase-by-phase breakdown.
...
### Phase 1: load, parsing files into a `PzProject`
...
### Phase 2: compile, turning files into a graph
...
### Phase 3: validate, catching problems before moving any data
...
without touching a single data row. See [Validation and errors](/concepts/validation-and-errors/)
...
small wire protocol (a gRPC/protobuf control socket plus an Arrow IPC data plane), rather than
loaded into this process at all. The result is a `ConnectorRegistry` the engine can ask for
connector instances by name, indifferent to which hosting model backs any given one. See
[Connector architecture](/internals/connector-architecture/) for the full ABI and hosting story.

### Phase 5: plan, deciding how each edge moves data
...
### Phase 6: dispatch, the graph actually runs

This is the biggest phase; it gets its own section (§6).

### Phase 7: finalize, writing down what happened
...
### Phase 8: report and the exit code

`pz` prints one summary line:
...
The `cleaned …` line is automatic retention reporting what it swept (§7 above). The run also
ends with an exit code of fixed meaning: **0** all good, **1** some nodes failed, **2** your
config/SQL is invalid (nothing ran), **3** fatal (crash or cancellation). Scripts and CI can
branch on these.

## 6. Inside dispatch: the dispatcher and the four executors
...
**`PipelineExecutor`** (`Execution/PipelineExecutor.cs`) is the simplest one: execute the
...
stream (`IRunEvents` → `RunEventBus`, contract in [Run events](/reference/events/)), and what
...
6. [Architecture](/internals/architecture/): *why* it's built this way.

And keep [Diagrams](/internals/diagrams/) open: five presentation-grade diagrams of
...
- [Execution internals](/internals/execution-internals/): the compiler, planner, and dispatcher in more depth.
- [Connector architecture](/internals/connector-architecture/): the ABI and PCP in full.
- [Resume internals](/internals/resume-internals/): the ledgers behind watermarks and `pz retry`.
- [Contributing](/internals/contributing/): the solution layout, build, and test commands.
