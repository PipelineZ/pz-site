...
and `pz mcp`'s stdio handshake. It's Linux-only because Native AOT can't cross-compile between
operating systems and the script is bash. The PCP path itself is also covered on Windows, by the
`build-test` windows leg (see CI, below). It's also a required PR gate.
...
| `rust` | ubuntu + windows matrix | Both legs run `cargo clippy -D warnings`, `cargo test` over `rust/`, then `scripts/rust-conformance.sh` against the .NET host (the conformance checks plus the `Category=RustPcp` telemetry tests). The windows leg exists because the SDK's socket transport has a Windows-only half. `cargo fmt --check` and `cargo audit` (RUSTSEC advisories) are platform-independent and run on ubuntu only. |
