...
| Code | Name | Meaning | Where it surfaces |
|---|---|---|---|
| `PZ0311` | NativeSetupFailed | A DuckDB native setup statement failed: an extension load, a secret, a session setting, or an attach. Includes the `excel`/`avro` extensions xlsx/avro need, whose first use on a machine and DuckDB version downloads them — with the hint "first use needs network access to install the DuckDB &lt;ext&gt; extension" when that install has no network to reach. The message never includes a credential. | Running a node on the native tier |
...
| `PZ0361` | FileFormatUnsupported | A file-place connector (localfiles, s3, gcs, azureblob, sftp) was asked for a `format:` it does not support: an unknown name, `format` missing where the connector has no default, a read-only format asked to write (`avro`, always; any format asked to write on sftp or a managed/universal write path), an `xlsx` write asked on s3/gcs/azureblob (localfiles-only, because DuckDB's excel writer aborts the whole process on a failed remote write), or a native-only format/option (json `layout: array`, `xlsx`, `avro`) asked on a connector or tier with no native path — sftp refuses `xlsx`/`avro` outright, and an `xlsx` read matching more than one file (a glob or date-templated `path:`) is refused too, since `read_xlsx` reads exactly one workbook. The "supported:" set in the message is `avro, csv, json, parquet, tsv, xlsx`. | Planning execution |
| `PZ0362` | FileFormatOptionInvalid | A format-scoped option is declared on a format that does not admit it (`delimiter` on tsv or parquet, `layout` on csv or parquet, `sheet`/`header` on any format but xlsx), or carries an invalid value (a multi-character or quote/newline/carriage-return `delimiter`, a `layout` other than `ndjson`/`array`, a `header` that is not a boolean, an empty `sheet` name). | Planning execution |
| `PZ0363` | SyncStateNativeOnly | A token-resumed entity (feed-shaped, or `sync: {mode: cdc}`) sits on a connector whose read path is native-only; a native scan never drains the partition the sync token is captured from, so the entity could never advance. A native-capable connector with a universal path is simply routed to that path instead. Use a connector with a universal read path, or read the entity as full or cursor-incremental. | Planning execution |
