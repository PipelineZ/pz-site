...
| `PZ0361` | FileFormatUnsupported | A file-place connector (localfiles, s3, gcs, azureblob, sftp) was asked for a `format:` it does not support: an unknown name, `format` missing where the connector has no default, a read-only format asked to write (`avro`, always; any format asked to write on sftp or a managed/universal write path), or a native-only format/option (json `layout: array`, `xlsx`, `avro`) asked on a connector or tier with no native path — sftp refuses `xlsx`/`avro` outright, and an `xlsx` read matching more than one file (a glob or date-templated `path:`) is refused too, since `read_xlsx` reads exactly one workbook. The "supported:" set in the message is `avro, csv, json, parquet, tsv, xlsx`. | Planning execution |
