...
| `pz_entity_schema` | `connection` (string), `entity` (string), `read?` (object) | `columns[]` of `{name, type}`, `source`: `"fetched"` when the columns came from a live connectivity probe (a contract-less entity), or `"declared_contract"` when they came from the entity's own YAML `columns:` contract (`ConnectivityValidator` never populates a live fetch for an entity that already declares one). Type vocabularies differ between the two: `"fetched"` types are Arrow-derived (`ContractTypes.Describe` strings, e.g. `Decimal128(10,2)`); `"declared_contract"` types are the literal YAML contract strings (e.g. `bigint`, `double`). `entity` need not already be declared: an undeclared entity of a declared connection is probed too, using `read` for whatever schema discovery needs beyond the bare name (e.g. `format: parquet` on localfiles, which otherwise defaults an undeclared entity to csv). `read` passed for an entity that *is* already declared is ignored — its own declared read applies instead — and the result carries a `note` saying so. `source: "declared_contract"` additionally carries `differs_from_contract` (bool) and, when true, `extra_columns[]` (string names): a clean probe already proves every declared column matches the live schema, so the one way a contract-bearing entity's live shape can still differ is a connector whose fetch discovers MORE columns than the contract declares (contracts prune on read, never widen). Unknown connection/entity, or a csv/json entity with neither a live fetch nor a declared contract (see [Detect schema drift at run time](/how-to/schema-drift/)), comes back as `PZ0330`. Opens a real connection; still read-only — never writes `.pz/target/schemas.json`. |
...
| `PZ0607` | The documentation tools could not reach the documentation site, or `PZ_DOCS_URL` names a `file:` mirror (a local directory tree, root-confined) that is missing or unreadable on disk — treated the same as an unreachable http one, not as a `pz` defect. The message names the URL that failed; the `next_step` points at `PZ_DOCS_URL` for a mirror. Only `pz_docs_*` can raise this — every other tool works offline. |
...
| `PZ0610` | A documentation response — the `llms.txt`/`llms-full.txt` fetch, or an individual page, over either http or a `file:` mirror — exceeded the 25 MB cap the docs tools refuse to consume. Distinct from `PZ0607`: the source WAS reached. Never a silent truncation. |
| `PZ0611` | `pz mcp init`'s target client config file (`.vscode/mcp.json` and similar) legally carries comments or trailing commas (JSONC), which a merge-and-rewrite through `JsonNode` cannot preserve — so the file is left untouched and the `next_step` pastes in the exact entry to add by hand. Distinct from `PZ0605`: the file is recognized, not broken. |
