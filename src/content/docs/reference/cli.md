...
| `pz cdc status` | Report server-side change-capture state for every `sync: {mode: cdc}` dataset in the project — whether the source-side capture object still exists and whether `pz`'s stored position is still readable from it. Exits `0` when every reported dataset is healthy, `1` when any is not; see [Capture changes with CDC](/how-to/capture-changes-with-cdc/) |
| `pz cdc drop <source>.<dataset>` | Drop the server-side change-capture state for **one** cdc dataset and clear `pz`'s sync-state entry for it (in whichever store `state:` resolved to), so the next run re-snapshots that dataset in full. Exactly one target — there is no bulk drop |
...
| `pz mcp [--allow-run]` | Serve the current project to AI agents over the Model Context Protocol (stdio only). Registers the introspect/verify/author/docs tools; `--allow-run` additionally exposes `pz_run`, `pz_retry`, and `pz_run_results`, which move real data. stdout belongs to the protocol — everything the CLI would print goes to the client as a log. See [Use pz with an AI agent](/how-to/use-with-an-ai-agent/) and the [MCP contract](/reference/mcp-contract/) |
| `pz mcp init [<client>...] [--all] [--allow-run] [--skill-locations <list>]` | Write MCP client config for `vscode`, `claude-code`, `copilot-cli`, and/or `opencode` (merge-preserving — an existing config keeps its other servers) and install the `pz-pipelines` skill. `--all` wires all four |
