---
title: "Chapter 9 - Best practices"
sidebar:
  order: 9
---
Eight chapters of machinery deserve a synthesis. This chapter is the habits - most of them
already earned the hard way somewhere in Chapters 1–8 - that separate pipelines people
...
repository, like any other software. This one habit quietly delivers half the chapter:
...
Everything hard in Chapters 3–6 gets easy on top of it - retries (rerun freely), backfills
(rerun the past), disaster recovery (rerun the world), debugging (rerun and watch).

The recipe, assembled from earlier chapters:

- Idempotent loads: **merge or replace**, never bare append, unless duplicates were
  consciously accepted (Chapter 4).
- **Deterministic** transforms: no `NOW()` inside logic; "as of when" is an input
  (Chapter 5).
- Watermarks advance **only after** delivery commits (Chapter 4).
- Rebuild tables from upstream; never patch them in place (Chapter 5).
...
- **Land raw before transforming** (Chapter 2's landing zone). Storage is cheap;
  re-extracting data the source no longer has is impossible. Raw data is your undo button
  and your evidence locker.
- **Many small steps over one clever step** (Chapter 5's layers). Small steps are testable
...
The two-sentence summary of Chapters 7–8, stated as practice:
...
- Fail loudly or not at all. The forbidden option is the *silent* failure - the caught
  exception that logs nothing, the empty extract that "succeeds" (Chapter 7's villain).
- Respect the systems you read from: off-peak schedules, incremental extraction, pushdown,
  concurrency caps, backoff on throttling (Chapter 4). Pipelines borrow other people's
...
Checks (Chapter 7) validate data at runtime. Also test the *logic* before it ships:
...
The quiet theme of the whole book. Batch before streaming (Chapter 3). Full refresh before
incremental (Chapter 3). Cron before an orchestration platform (Chapter 6). Three layers
